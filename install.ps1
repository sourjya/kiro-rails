...
# Usage: irm https://raw.githubusercontent.com/sourjya/kiro-rails/main/install.ps1 | iex
# Or:    .\install.ps1
...
function Download-File($relativePath) {
    ...
    try {
        ...
        Write-Host "  Warning: could not download $relativePath" -ForegroundColor Yellow
        return $false
    }
}
...
foreach ($file in $ManagedFiles) {
    ...
    if ((Test-Path $localPath) -and $installType -eq "upgrade") {
        if (Download-File $file) { $updated++ } else { $failed++ }
    } else {
        if (Download-File $file) { $downloaded++ } else { $failed++ }
    }
}
...
if (Test-Path $overridesLocal) {
    ...
    Download-File $OverridesFile.Replace("\", "/") | Out-Null
    $downloaded++
    ...
    if ([Environment]::UserInteractive) {
        ...
        if ($configure -eq "" -or $configure -match "^[Yy]") {
            ...
            Set-Content -Path $overridesLocal -Value $content -NoNewline
            Write-Host ""
...
# ── Write version file ──
$CurrentVersion | Set-Content -Path ($VersionFile.Replace("/", "\")) -NoNewline
