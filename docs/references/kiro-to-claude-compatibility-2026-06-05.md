...
| `.kiro/agents/*.json` | `.claude/agents/*.md` (subagents) | JSON -> markdown frontmatter (`name`, `description`, `tools`) + body from the referenced `prompt` file. Tool names remap: `read`->`Read`, `grep`->`Grep`, `glob`->`Glob`, `shell`->`Bash`; `knowledge` and `code` have no Claude tool (dropped, noted on stderr). **Fails closed:** if no tool maps, the agent is emitted with `tools: Read` rather than no `tools:` line - an agent with no `tools:` frontmatter would inherit *every* tool, silently turning a sandboxed reviewer into a fully privileged one. |
