...
- [ ] 2026-06-05 | Reconcile install.ps1 drift vs install.sh - ps1 is missing the spec-* skills, export-to-tools.sh, and the agents, so Windows installs are incomplete for those (pre-existing; the v0.10-0.12 features ARE wired into both) | context: noticed while wiring the new features into the installers
