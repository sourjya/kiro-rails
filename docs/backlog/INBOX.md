...
- [ ] 2026-06-05 | Installer leaves install.sh behind after `curl ... | bash` on Linux - it should clean up after itself. Windows command already does `Remove-Item install.ps1`; verify/standardize cleanup for both | context: reported while fixing install.ps1 parity
